...
export const loginParentDataInput = async (req: Request, res: Response) => {
  const { parentId, parentName, parentEmail } = req.body;

  try {
    const [result]: any = await promisePool.query(
      'INSERT INTO parent (parent_id, parent_name, parent_email) VALUES(?,?,?)',
...
export const forgotPassword = async (req: Request, res: Response) => {
  ...
  try {
    const [rows]: any = await promisePool.query(
      'SELECT * FROM smartle.parent WHERE parent_email = ?;',
      [email]
    );
    if (rows?.length === 0) {
...
export const childrenSelect = async (req: Request, res: Response) => {
...
export const createChild = async (req: Request, res: Response) => {
  ...
  try {
    const [result]: any = await promisePool.query(
      'INSERT INTO student (student_name, student_gender, student_age, parent_id) VALUES(?,?,?,?)',
      [studentName, studentGender, studentAge, parentId, studentGender]
    );
    res.json({ message: 'success', result }).end();
  } catch (error) {
    res.status(404).json({ message: 'Error' });
